...
| 层 | 技术 |
|----|------|
| 后端 | Go 1.25, SQLite (modernc.org/sqlite) / PostgreSQL 17, gorilla/websocket, goja (JS VM) |
...
It turns WeChat's raw messaging capability into a manageable, routable, and extensible system: bind multiple WeChat accounts via QR code, then forward messages to your services through WebSocket, Webhook (with JavaScript middleware), or AI auto-reply.
...
**Key Features:** Multi-bot management, built-in SQLite (zero-config) + optional PostgreSQL, Passkey (WebAuthn) passwordless login, GitHub/LinuxDo OAuth, JavaScript plugin engine, plugin marketplace, and official SDKs for Go, Node.js, Python, PHP, Java, C#, and Lua.
